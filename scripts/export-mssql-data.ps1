...
$tables = @(
    ...
    @{Name="Plan"; Query="SELECT PlanId, Name, MealCount, FuelingCount, CondimentCount FROM Plan"},
    @{Name="Fueling"; Query="SELECT FuelingId, Name, Disabled FROM Fueling"},
    ...
    @{Name="UserFueling"; Query="SELECT UserFuelingId, UserId, FuelingId, Day, [When] FROM UserFueling"},
    @{Name="UserMeal"; Query="SELECT UserMealId, UserId, Day, Type, [When] FROM UserMeal"},
